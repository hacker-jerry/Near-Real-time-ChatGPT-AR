...
//        loadLocalModel(name: name) //加载本地模型
        loadUSDZ(name: name)
    }

    private func loadLocalModel(name: String){
        guard let path = Bundle.main.path(forResource: name, ofType: "zip", inDirectory: "Models/airplane") else {
    ...
    private func loadUSDZ(name: String){
        var folderName = ""
        var modelName = ""
        var fileurl = ""
        
        if name.lowercased().contains("dog") {
            folderName = "dog"
            modelName = "dog"
            fileurl = "Models/dog/dog.usdz"
        } else if name.lowercased().contains("model") {
            folderName = "airplane"
            modelName = "airplane"
            fileurl = "Models/trailer/trailer.usdz"
        } else {
            fileurl = "Models/trailer/trailer.usdz"
        }
        
        guard let fileURL = Bundle.main.url(forResource: "airplane", withExtension: "usdz", subdirectory: "Models/airplane_usdz") else {
            fatalError("Failed to find model file.")
        }

        print("成功读取文件：\(fileurl)")
        do {
            let asset = MDLAsset(url: fileURL)
//            let scene = SCNScene(mdlAsset: asset)
//            let scene = SCNScene(named: "Models/airplane_usdz/airplane2.usdz")
//            let scene = SCNScene(named: "Models/cute_girl/girl.usdz")
            let scene = SCNScene(named: fileurl)
//            let entity = try! ModelEntity.loadModel(named: "Models/airplane_usdz/airplane")
//            entity.generateCollisionShapes(recursive: true)
            
//            let scene = try usdzSource.scene()
...
//
//            newRootNode.addChildNode(spotLight)

            DispatchQueue.main.async { [weak self] in
                self?._rootNode.addChildNode(newRootNode)
            }
        } catch {
            print("[SketchfabEntity] Unable to load USDZ assset: \(error.localizedDescription)")
            return
        }
    }
